...
- The program is compiled with gcc 4.8.4 using `gcc`, using the options `-Wall -Werror -Wextra -pedantic -std=gnu89`
- The code is in format: `Betty` style.
- All headers are in `header.h`
- The program has no memory leaks in the parent process or in each child process.
- Simple_Shell has the output of sh (/bin/sh), as well as the same error output.
